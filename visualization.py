...
# Scatter Plot
def scatter_plot_aer(y_true, y_pred, file_name, save_path, model_mode = "No MB", sensor_name = None, mode_name=None, show_plot=True):
...
def scatter_plot(y_true, y_pred, file_name, save_path, model_mode = "No MB", sensor_name = None, mode_name=None, show_plot=True):
    ...
    save_file = os.path.join(save_path, f"{model_mode}_{file_name}.png")
    plt.savefig(save_file, dpi=600, bbox_inches='tight')
    print(f"--- Successfully saved scatter plot to: {save_file} ---")
    if show_plot:
        plt.show()
...
# Time Series Plot
def line_plot_aer(y_true, y_pred, file_name, save_path, model_mode = "No MB", sensor_name = None, mode_name=None, pred_steps=None, show_plot=True):
...
def line_plot(y_true, y_pred, file_name, save_path, model_mode = "No MB", sensor_name = None, mode_name=None, pred_steps=None, show_plot=True):
    ...
    plt.title(f'{model_mode} Time Series\n{sensor_name} | {mode_name}', fontsize=14, fontweight='bold')
    plt.xlabel('Time Steps', fontsize=14, fontweight='bold')
    plt.ylabel('Concentration (µg/m3)', fontsize=14, fontweight='bold')
    ax = plt.gca()
    for label in ax.get_xticklabels() + ax.get_yticklabels():
